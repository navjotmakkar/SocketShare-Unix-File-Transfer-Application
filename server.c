...
#define S_TYPE_CONNECT 'S'
#define M_IP_PORT 40002
#define M_TYPE_CONNECT 'M'
#define M_IP_ADDR "0.0.0.0"
#define BUFFR_LEN_BYTES 2048
#define OUT_ZIP_NAME "out.tar.gz"
#define OUT_FLIST_NAME "outFilePathList.txt"
...
// !!! below function sends the client request to mirror server
int clientRedirectMirror(int server_sd){

    // structure to hold mirror address family details
    mirrorStruct mirrorObj;

    long typeResponse = 2; // for mirror redirection
    send(server_sd, &typeResponse, sizeof(typeResponse), 0);

    // !! setting up mirror IP address and prt number
    strcpy(mirrorObj.ipAddr, M_IP_ADDR);
    mirrorObj.portNo = M_IP_PORT;   

    // !! mirror server information is being sent to client 
    send(server_sd, &mirrorObj, sizeof(mirrorStruct), 0);
    printf("Request sent to client %d to connect to mirror. \n", numClientConnected);

}
// !! based on number of client - Determines which to connect SERVER or MIRROR ?
char checkServerOrMirror(int numClientConnected) {
    //static variable to store last connection type
    static char currentType = S_TYPE_CONNECT;
    
    //connect with server for client 1 to 6
    if (numClientConnected >= 1 && numClientConnected <= 6) {
        currentType = S_TYPE_CONNECT;
    } 
    //connect with mirror for client 7 to 12
    else if (numClientConnected >= 7 && numClientConnected <= 12) {
        currentType = M_TYPE_CONNECT;
    } 
    //toggle between server and mirror from client 13 onwards
    else {
        currentType = (currentType == S_TYPE_CONNECT) ? M_TYPE_CONNECT : S_TYPE_CONNECT;
    }
...
int process_fgets(int server_sd, char** fNamesArr, int numFNames){
    ...
    // Create tar command to compress files into out.tar.gz
    sprintf(buffrTarCmd, createTarCmd, OUT_ZIP_NAME, OUT_FLIST_NAME);
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
      ...
      // Determining to connect with server or to mirror 
      char connTypeFlag = checkServerOrMirror(numClientConnected);
      
      // request is redirected to Server
      if(connTypeFlag == S_TYPE_CONNECT){
        // accepting the request from the client
        conn_sd = accept(server_sd,(struct sockaddr *) NULL, NULL);
        printf("Conection request accepted for client %d\n", numClientConnected);
      ...
      // request is redirectted to mirror 
      else if(connTypeFlag == M_TYPE_CONNECT){
        // acceptance of client connection request
        conn_sd = accept(server_sd, (struct sockaddr *) NULL, NULL);
       
        // new process for request from client 
        if(!fork()){
            clientRedirectMirror(conn_sd);
            close(conn_sd);
            exit(0);
        }
         // waiting for child prcoesses to finish without blocking the loop
